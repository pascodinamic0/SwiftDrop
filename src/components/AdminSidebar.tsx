    ...
    <Sidebar collapsible="icon">
      <SidebarHeader className="border-b border-sidebar-border">
        <div className="flex items-center gap-2 px-2 py-1">
          {!collapsed ? <Logo /> : <div className="h-8 w-8 rounded-lg bg-primary flex items-center justify-center font-display font-bold text-primary-foreground">S</div>}
        </div>
      ...
      <SidebarFooter className="border-t border-sidebar-border">
        {!collapsed && user && <div className="px-2 py-1 text-xs text-muted-foreground truncate">{user.email}</div>}
        <Button variant="ghost" size="sm" className="justify-start gap-2" onClick={async () => { await signOut(); navigate({ to: "/" }); }}>
