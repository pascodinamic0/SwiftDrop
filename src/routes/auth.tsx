...
function AuthPage() {
  ...
  return (
    <div className="min-h-screen grid md:grid-cols-2">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="w-full max-w-sm">
            ...
            <p className="mt-2 text-muted-foreground">
              {mode === "login" ? "Sign in to send or accept deliveries." : "Start sending packages in under a minute."}
            </p>
      ...
      {/* Right: brand panel */}
      <div className="hidden md:flex relative bg-secondary text-secondary-foreground p-12 flex-col justify-between overflow-hidden">
        <div className="absolute inset-0 bg-grid opacity-10" />
        <div className="relative">
          <div className="text-xs uppercase tracking-widest text-primary font-semibold">Trusted by 12,000+ couriers</div>
          <h2 className="mt-4 font-display text-4xl font-bold leading-tight">
            The fastest way to move things across your city.
          </h2>
        </div>
        <div className="relative space-y-4">
          {["Pick a role after signup — Customer or Delivery Agent.", "Set your pickup and drop-off, see the price upfront.", "Track your courier (or drone) in real time."].map((s, i) => (
            <div key={i} className="flex gap-3 items-start">
              <div className="h-7 w-7 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-sm font-bold flex-shrink-0">{i + 1}</div>
