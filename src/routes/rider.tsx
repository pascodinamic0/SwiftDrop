...
export const Route = createFileRoute("/rider")({
  ...
  component: () => <RequireRole role="delivery_agent"><RiderShell /></RequireRole>,
});
...
function RiderShell() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      // Ensure rider_profile exists
      const { data } = await supabase.from("rider_profiles").select("is_online").eq("id", user.id).maybeSingle();
      if (!data) {
        await supabase.from("rider_profiles").insert({ id: user.id });
      } else {
        setOnline((data as { is_online: boolean }).is_online);
      }
    })();
  }, [user]);
  ...
  const toggleOnline = async (v: boolean) => {
    if (!user) return;
    setOnline(v);
    await supabase.from("rider_profiles").update({ is_online: v }).eq("id", user.id);
  };
