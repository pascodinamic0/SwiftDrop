...
function ActiveDelivery() {
  ...
  const load = async () => {
    ...
    if (data) {
      ...
      const { data: s } = await supabase
        ...
        .eq("id", (data as ActiveOrder).store_id)
        .single();
  ...
  const markDelivered = async () => {
    ...
    toast.success(t("rider.deliveredToast", { fee: Number(order.delivery_fee).toFixed(2) }));
  };
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-2xl">
      <Badge className="bg-primary text-primary-foreground">{t("rider.activeDelivery")}</Badge>
      ...
      <Card className="mt-4 p-5">
        <h3 className="font-semibold flex items-center gap-2">
          <MapPin className="h-4 w-4 text-primary" /> {t("rider.pickup")}
        </h3>
      ...
      <Card className="mt-3 p-5">
        <h3 className="font-semibold flex items-center gap-2">
          <MapPin className="h-4 w-4 text-secondary" /> {t("rider.dropoff")}
        </h3>
      ...
      <Card className="mt-3 p-5 bg-secondary text-secondary-foreground">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs opacity-70">{t("rider.cashToCollect")}</p>
            <p className="font-display text-3xl font-bold text-primary">
              ${Number(order.delivery_fee).toFixed(2)}
            </p>
          </div>
          <div className="text-right text-xs opacity-70">
            <p>{t("rider.itemsPrepaid")}</p>
      ...
      <Button variant="hero" size="lg" className="w-full mt-4" onClick={markDelivered}>
        <CheckCircle className="h-5 w-5" /> {t("rider.markDelivered")}
      </Button>
    </div>
  );
}
