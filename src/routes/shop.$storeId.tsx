...
interface Store { id: string; name: string; description: string | null; image_url: string | null; address: string; delivery_fee: number; is_open: boolean; category: string; }
interface Cat { id: string; name: string; sort_order: number; }
interface Item { id: string; name: string; description: string | null; price: number; image_url: string | null; category_id: string | null; is_available: boolean; }
...
function StorePage() {
  ...
  if (!store) return <div className="min-h-screen flex flex-col"><SiteHeader /><div className="p-12 text-center text-muted-foreground">Loading…</div></div>;

  const handleAdd = (it: Item) => {
    if (!store.is_open) { toast.error("This store is currently closed."); return; }
    addItem(store.id, store.name, { id: it.id, name: it.name, price: Number(it.price), image_url: it.image_url });
      ...
      <div className="container mx-auto px-4 -mt-20 relative">
        <Card className="p-6">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <Badge className="capitalize mb-2">{store.category}</Badge>
              ...
              <p className="mt-2 text-sm text-muted-foreground flex items-center gap-1"><MapPin className="h-3.5 w-3.5" />{store.address}</p>
            </div>
            <div className="text-right">
              <div className="text-xs text-muted-foreground">Delivery fee (cash on arrival)</div>
              <div className="font-display text-2xl font-bold">${Number(store.delivery_fee).toFixed(2)}</div>
            </div>
          </div>
        </Card>
                    ...
                    <Card key={it.id} className="p-4 flex gap-4 hover:shadow-card transition-all">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-2 flex items-center justify-between">
                          <span className="font-display text-lg font-bold">${Number(it.price).toFixed(2)}</span>
          ...
          <div className="container mx-auto">
            <Link to="/cart">
              <Button variant="hero" size="lg" className="w-full justify-between shadow-glow">
                <span className="flex items-center gap-2"><ShoppingBag className="h-5 w-5" /> {count} item{count > 1 ? "s" : ""} in cart</span>
